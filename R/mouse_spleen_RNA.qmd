...
library(grid)
library(gridExtra)
# library(Giotto) # nice for fov plots but has some hurdles due to changing file structure from cosMx.  Also, the morphology composite images are no longer available.
```
...
mouseIO_colors <- c("#1b9e77", "#7570b3", "#e7298a", "#66a61e", "#e6ab02",
                    ...
                    "#cccccc", "#dddddd", "#555555",
                    "#444444", "#555555", "#666666", "darkgreen", "#ff7f00",
...
names(mouseIO_colors) <- c("B.cell", "Dendritic.cell", "Endothelial", 
                            ...
                            "unknown", "Unknown.34", "Unknown.35",
                           "Unknown.7","Unknown.9", "Unknown.14", "B.cell.like", "Macrophage/Monocytes",
                            "a","b","c","d", "e")
...
# critical
# This splits the object into layers including counts.2.Split2, counts.2.Split1, same for data
sobj_spleen_split2[["RNA"]] <- split(sobj_spleen_split2[["RNA"]], f = sobj_spleen_split2$Split) 
sobj_spleen_split2
...
sobj_spleen_split2 <- FindClusters(sobj_spleen_split2, resolution = 2, cluster.name = "unintegrated_clusters")
...
sobj_spleen_split2 <- FindNeighbors(sobj_spleen_split2, reduction = "integrated.cca", dims = 1:30)
sobj_spleen_split2 <- FindClusters(sobj_spleen_split2, resolution = 1)
sobj_spleen_split2 <- RunUMAP(sobj_spleen_split2, dims = 1:30, reduction = "integrated.cca")
# Visualization
...
# 11 ?
# 12
# 13 (cb DC) and 16 were Neutrophils
# 15 Plasma
# 17 macrophage 
# 18 disappears in Homoz. 
# 19 left protrusion - more B cells
...
df_meta <- df_meta %>%
  mutate(CellType_Integrated = case_when(
    ...
    seurat_clusters  == 1 ~ "1",
    seurat_clusters  == 2 ~ "2",
    seurat_clusters == 3 ~ "Erythroid.Projenitor",
    seurat_clusters  == 4 ~ "T.cell",
    seurat_clusters  == 5 ~ "T.cell.2",
    seurat_clusters == 6 ~ "6",
    seurat_clusters  == 7 ~ "T.cell.3",
    seurat_clusters  == 8 ~ "B.cell.like",
    seurat_clusters == 9 ~ "Dendritic.cell",
    seurat_clusters  == 10 ~ "10",
    seurat_clusters  == 11 ~ "11",
    seurat_clusters == 12 ~ "12",
    seurat_clusters  == 13 ~ "Neutrophil.like",
    seurat_clusters  == 14 ~ "B.cell",
    seurat_clusters  == 15 ~ "Plasma",
    seurat_clusters == 16 ~ "Neutrophil",
    seurat_clusters  == 17 ~ "Macrophage",
    seurat_clusters  == 18 ~ "18",
    seurat_clusters  == 19 ~ "B.cell.2",
    TRUE ~ "other" # Optional: handle unexpected values
  ))
...
table(sobj_spleen5@meta.data$CellType_Integrated)

bcell.markers <- Seurat::FindConservedMarkers(sobj_spleen5, ident.1 = "B.cell", grouping.var = "Split", verbose = FALSE)
head(bcell.markers)

neutrophil.markers <- Seurat::FindConservedMarkers(sobj_spleen5, ident.1 = "Neutrophil", grouping.var = "Split", verbose = FALSE)
head(neutrophil.markers)

dc.markers <- Seurat::FindConservedMarkers(sobj_spleen5, ident.1 = "Dendritic.cell", grouping.var = "Split", verbose = FALSE)
head(dc.markers)
...
for (i in 0:19) {
  cons_markers[[i+1]] <- Seurat::FindConservedMarkers(sobj_spleen5, ident.1 = i, grouping.var = "Split", verbose = FALSE)
...
Idents(sobj_spleen_split2) <- factor(Idents(sobj_spleen_split2), levels = c("pDC", "Eryth", "Mk", "DC", "CD14 Mono", "CD16 Mono",
    "B Activated", "B", "CD8 T", "NK", "T activated", "CD4 Naive T", "CD4 Memory T"))

markers.to.plot <- c("CD3D", "CREM", "HSPH1", "SELL", "GIMAP5", "CACYBP", "GNLY", "NKG7", "CCL5",
    "CD8A", "MS4A1", "CD79A", "MIR155HG", "NME1", "FCGR3A", "VMO1", "CCL2", "S100A9", "HLA-DQA1",
    "GPR183", "PPBP", "GNG11", "HBA2", "HBB", "TSPAN13", "IL3RA", "IGJ", "PRSS57")
DotPlot(sobj_spleen_split2, features = markers.to.plot, cols = c("blue", "red"), dot.scale = 8, split.by = "stim") +
    RotatedAxis()
