...
df_meta <- df_meta %>%
  mutate(CellType_Integrated = case_when(
    ...
    seurat_clusters  == 5 ~ "c", # ?
    seurat_clusters == 6 ~ "a", # mostly sample 35 
    seurat_clusters  == 7 ~ "b", # Platelets or basophiles
    seurat_clusters  == 8 ~ "c", # Basophil.like",
    ...
    seurat_clusters == 16 ~ "d",
    seurat_clusters  == 17 ~ "d", # "Macrophage" had only 2,
    seurat_clusters  == 18 ~ "d", # 2 cells only
    seurat_clusters  == 19 ~ "d", # 2 cells only
    TRUE ~ "other" # Optional: handle unexpected values
  ))
...
DimPlot(sobj_spleen_split2, reduction = "umap", cols=cols, label=TRUE)                                                                            
...
Idents(sobj_spleen_split2) <- factor(sobj_spleen_split2@meta.data$CellType_Integrated, levels = c(  "e","d","c", "b","a",  "Granulocyte", "Fibroblast", "Endothelial",  "Erythroid.Projenitor", "Macrophage", "Monocyte",  "Dendritic.cell", "Neutrophil" ,"T.cell" , "T.cell.CD8","Plasma","B.cell" ))
table(Idents(sobj_spleen_split2))
...
"Thbs1",   "Myl9",    "Hmgb2",   "Stmn1",   "Ube2c" ,  "Mt1" ,    "Top2a"  ,
...
FeaturePlot(sobj_spleen_split2, c("Ltf"))
FeaturePlot(sobj_spleen_split2, c("Acta2", "Clu", "Tagln"))
...
bootstrap_ci <- function(data, n_bootstrap = 1000, ci = 0.95) {
  boot_medians <- replicate(n_bootstrap, {
    sample_medians <- sample(data$percent_of_cells_positive, replace = TRUE)
    median(sample_medians)
  })
  quantile(boot_medians, probs = c((1 - ci) / 2, 1 - (1 - ci) / 2))
}
...
summary_df <- df %>%
  group_by(condition) %>%
  summarise(
    median = median(percent_of_cells_positive),
    ci_lower = bootstrap_ci(cur_data(), 1000, 0.95)[1],
    ci_upper = bootstrap_ci(cur_data(), 1000, 0.95)[2]
  )
...
counts <- as.matrix(GetAssayData(sobj, assay="RNA", layer="counts"))
data <- as.matrix(GetAssayData(sobj, assay="RNA", layer="data"))
