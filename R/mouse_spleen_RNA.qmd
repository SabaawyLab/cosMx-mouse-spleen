...
df_30_ct_neut <- get_pct_fov_celltype(df_ct30, 30, 'Neutrophil', 'Ccl5')
df_30_ct_dc <- get_pct_fov_celltype(df_ct30, 30, 'Dendritic.cell', 'Cxcl13')
df_30_ct_neut
df_30_ct_dc

df_30_ct_test <- get_pct_fov_celltype(df_ct30, 30, 'Dendritic.cell', 'Ccl5')
df_30_ct_test
...
df_31_ct_neut <- get_pct_fov_celltype(df_ct31, 31, 'Neutrophil', 'Ccl5')
df_31_ct_dc <- get_pct_fov_celltype(df_ct31, 31, 'Dendritic.cell', 'Cxcl13')

df_32$fov <- sapply(df_32$cell, parse_third_token)
df_ct32 <- merge(df_32, df_cell_types, by="cell")
df_32_ct_neut <- get_pct_fov_celltype(df_ct32, 32, 'Neutrophil', 'Ccl5')
df_32_ct_dc <- get_pct_fov_celltype(df_ct32, 32, 'Dendritic.cell', 'Cxcl13')

df_34$fov <- sapply(df_34$cell, parse_third_token)
df_ct34 <- merge(df_34, df_cell_types, by="cell")
df_34_ct_neut <- get_pct_fov_celltype(df_ct34, 34, 'Neutrophil', 'Ccl5')
df_34_ct_dc <- get_pct_fov_celltype(df_ct34, 34, 'Dendritic.cell', 'Cxcl13')

df_35$fov <- sapply(df_35$cell, parse_third_token)
df_ct35 <- merge(df_35, df_cell_types, by="cell")
df_35_ct_neut <- get_pct_fov_celltype(df_ct35, 35, 'Neutrophil', 'Ccl5')
df_35_ct_dc <- get_pct_fov_celltype(df_ct35, 35, 'Dendritic.cell', 'Cxcl13')
df_35_ct_neut
df_35_ct_dc
...
#| message: false
#| warning:false
df_genes_by_dc_by_fov <- rbind(df_30_ct_dc, df_31_ct_dc, df_32_ct_dc, df_34_ct_dc, df_35_ct_dc)
df_genes_by_neut_by_fov <- rbind(df_30_ct_neut, df_31_ct_neut, df_32_ct_neut, df_34_ct_neut, df_35_ct_neut)
...
df_ct30_sample_dc <- get_sample_pcts_bycelltype(df_ct30, 30, celltype='Dendritic.cell', marker='Cxcl13')
df_ct31_sample_dc <- get_sample_pcts_bycelltype(df_ct31, 31, celltype='Dendritic.cell', marker='Cxcl13')
df_ct32_sample_dc <- get_sample_pcts_bycelltype(df_ct32, 32, celltype='Dendritic.cell', marker='Cxcl13')
df_ct34_sample_dc <- get_sample_pcts_bycelltype(df_ct34, 34, celltype='Dendritic.cell', marker='Cxcl13')
df_ct35_sample_dc <- get_sample_pcts_bycelltype(df_ct35, 35, celltype='Dendritic.cell', marker='Cxcl13')

df_ct30_sample_neut <- get_sample_pcts_bycelltype(df_ct30, 30, celltype='Neutrophil', marker='Ccl5')
df_ct31_sample_neut <- get_sample_pcts_bycelltype(df_ct31, 31, celltype='Neutrophil', marker='Ccl5')
df_ct32_sample_neut <- get_sample_pcts_bycelltype(df_ct32, 32, celltype='Neutrophil', marker='Ccl5')
df_ct34_sample_neut <- get_sample_pcts_bycelltype(df_ct34, 34, celltype='Neutrophil', marker='Ccl5')
df_ct35_sample_neut <- get_sample_pcts_bycelltype(df_ct35, 35, celltype='Neutrophil', marker='Ccl5')


# rbind
df_dc_by_sample <- rbind(df_ct30_sample_dc, df_ct31_sample_dc, df_ct32_sample_dc, df_ct34_sample_dc, df_ct35_sample_dc)
df_neut_by_sample <- rbind(df_ct30_sample_neut,df_ct31_sample_neut,df_ct32_sample_neut,df_ct34_sample_neut,df_ct35_sample_neut)

write.csv(df_dc_by_sample, paste0(results_dir, "/Dendritic_by_sample.csv"))
write.csv(df_neut_by_sample, paste0(results_dir, "/Neutrophils_by_fov.csv"))
...
p <- DotPlot(sobj_spleen5, features = top_genes$gene) + RotatedAxis()
p
...
# Cluster renames
# Rename the cluster
df_meta <- sobj_spleen5@meta.data[c('cell', 'seurat_clusters')]
df_meta$seurat_clusters.6 <- df_meta$seurat_clusters
head(df_meta)

df_meta <- df_meta %>%
  mutate(CellType_Label6 = case_when(
    seurat_clusters.6 == 0 ~ "Unknown.35",
    seurat_clusters.6  == 1 ~ "B.cell",
    seurat_clusters.6  == 2 ~ "Erythroid.Projenitor",
    seurat_clusters.6 == 3 ~ "T.cell",
    seurat_clusters.6  == 4 ~ "B.cell",
    seurat_clusters.6  == 5 ~ "Unknown.34",
    seurat_clusters.6 == 6 ~ "B.cell.like",
    seurat_clusters.6  == 7 ~ "Unknown.7",
    seurat_clusters.6  == 8 ~ "Macrophage/Monocytes",
    seurat_clusters.6 == 9 ~ "Unknown.9",
    seurat_clusters.6  == 10 ~ "Neutrophils",
    seurat_clusters.6  == 11 ~ "Plasma",
    seurat_clusters.6 == 12 ~ "Dendritic.cell",
    seurat_clusters.6  == 13 ~ "Macrophage",
    seurat_clusters.6  == 14 ~ "Unknown.14",
    seurat_clusters.6  == 15 ~ "Dendritic.cell",
    seurat_clusters.6  == 16 ~ "B.cell",
    TRUE ~ "other" # Optional: handle unexpected values
  ))

sobj_spleen5 <- AddMetaData(sobj_spleen5, metadata = df_meta[c('seurat_clusters.6','CellType_Label6')])
...
get_top_markers <- function(cluster, n) {
  # depends on Idents being set correctly 
  markers <- FindMarkers(sobj_spleen5, ident.1 = cluster)
  markers %>%
...
DotPlot(sobj_spleen5, features = unique(markers_to_plot$gene)) + RotatedAxis() +
  ...
  ggtitle("Seurat Unsupervised Umap clusters Res6 top Log2FC")
...
T.cells may be able to be clustered further.

```{r subcluster}
#| echo: false
#| warning: false

# Step 1: Subset Seurat object for cluster 4
Idents(sobj_spleen5) <- "seurat_cluster.5"
table(Idents(sobj_spleen5))
cluster_4_cells <- subset(sobj_spleen5, subset = seurat_cluster.5 == 4) # Replace '4' with the cluster number for T cells
dim(cluster_4_cells)

# Step 2: Re-normalize and prepare the subsetted data
...
cluster_4_cells <- cluster_4_cells %>%
  ...
  FindClusters(resolution = 0.3) %>% # Adjust resolution for desired granularity
  RunUMAP(dims = 1:10)

# Step 4: Visualize UMAP of subclusters
DimPlot(cluster_4_cells, reduction = "umap", label = TRUE) + ggtitle("Subclusters of Cluster 4")

# Step 5: Add subcluster information back to the original Seurat object
sobj_spleen5$Tcell_subcluster <- NA
sobj_spleen5$Tcell_subcluster[Cells(cluster_4_cells)] <- Idents(cluster_4_cells)

```

```{r tcelldot}

# T.cell subset dot plot
table(Idents(cluster_4_cells))
# same as?
table(cluster_4_cells@meta.data$seurat_clusters)
clusters <- unique(Idents(cluster_4_cells))
...
genes_lineage <- c("Cd19", "Ptprc", "Cd8a", "Cd8b1", "Cd68", "Foxp3","Cd163","Itgax", "Itgam", "Cd3e", "Cd3d", "Fn1", "Acta2")
...
DotPlot(cluster_4_cells, features = unique(markers_to_plot$gene)) + RotatedAxis() +
  theme_minimal() + # Change to a minimal (white) background
  theme(
    axis.text.x = element_text(size = 8, angle = 45, hjust = 1), # Reduce font size and rotate x-axis labels
    axis.text.y = element_text(size = 10), # Adjust y-axis font size if needed
  ...
  ggtitle("TCell types top Log2FC")

ggsave(paste0(results_dir, "/Tcells_DotPlot.png"), last_plot(), width = 12)
...
#ClusterDistrBar(sobj_spleen5$orig.ident, sobj_spleen5$seurat_cluster.5)

# how to group 
SeuratExtend::ClusterDistrBar(sobj_spleen5$orig.ident, sobj_spleen5$CellType_sup)

# WT Samples only
# how to group 
SeuratExtend::ClusterDistrBar(sobj_spleen30$orig.ident, sobj_spleen30$CellType_sup)
SeuratExtend::ClusterDistrBar(sobj_spleen30$orig.ident, sobj_spleen30$CellType_Label)
```

```{r}
SeuratExtend::ClusterDistrBar(sobj_spleen30$orig.ident, sobj_spleen32$CellType_Label)
...
run_pca_stuff <- function(sobj) {
  ...
  sobj <- FindClusters(sobj,  cluster.name='seurat_cluster.4', resolution=0.4, verbose = FALSE)
...
DimPlot(sobj_spleen32, reduction = "umap", alpha=0.5, cols='polychrome')
Idents(sobj_spleen34) <- "CellType_Label"
DimPlot(sobj_spleen34, reduction = "umap", alpha=0.5, cols='polychrome')
...
which_cluster <- "seurat_cluster.5"
...
# Function to find top n markers for a specific cluster
n <- 15
get_top_markers <- function(cluster) {
  markers <- FindMarkers(sobj_spleen5, ident.1 = cluster)
  markers %>%
    rownames_to_column(var = "gene") %>% # Add gene names as a column
    top_n(n, wt = abs(avg_log2FC)) %>%  # Select top 10 markers by absolute log2 fold change
...
# Apply the function to all clusters and combine results
top_markers <- map_dfr(clusters, get_top_markers)
...
# change to my labels 
mouseIO_colors <- c("#1b9e77", "#7570b3", "#e7298a", "#66a61e", "#e6ab02",
                    "#a6761d", "#b3cc1a", "#377eb8", "#f87878",  "#4daf4a", "#984ea3",
                    "#a65628", "#af648b", "#999999", "#999999",
                    "#ff7f00", "#ff7f00", "turquoise", "lightblue",
                    "#cccccc",
                    "lightgreen", "turquoise", "lightblue", "peachpuff", "pink")

names(mouseIO_colors) <- c("B.cell", "Dendritic.cell", "Endothelial", 
                            "Fibroblast", "Macrophage", "Mast.cell",
                            "Monocyte", "Neutrophil", "NK.cell", "Plasma",
                            "Plasmablast", "T.cell.CD4", "T.cell.CD8",
                            "T.cell.regulatory", "T.cell.reg",
                            "Plasmacytoid.dendritic.cell", "pDC",
                            "Myofibroblast", "Erythroid.projenitor",
                            "unknown",
                            "a","b","c","d", "e")
...
# Feature Plots
Idents(sobj_spleen5) <- "CellType_Label"
ImageDimPlot(sobj_spleen5, fov="fov30", border.size = 0.1,
             cols = 'polychrome',
             axes = TRUE,
             flip_xy = FALSE,
             coord.fixed=TRUE) 

ImageDimPlot(sobj_spleen5, fov="fov31", border.size = 0.1,
             cols = 'polychrome',
             axes = TRUE,
             flip_xy = FALSE,
             coord.fixed=TRUE) 

ImageDimPlot(sobj_spleen5, fov="fov32", border.size = 0.1,
             cols = 'polychrome',
             axes = TRUE,
             flip_xy = FALSE,
             coord.fixed=TRUE) 

ImageDimPlot(sobj_spleen5, fov="fov34", border.size = 0.1,
             cols = 'polychrome',
             axes = TRUE,
             flip_xy = FALSE,
             coord.fixed=TRUE) 
...
SeuratExtend::ClusterDistrBar(sobj_spleen5$orig.ident, sobj_spleen5$CellType_InSitu4,
                              cols = mouseIO_colors)
...
feature_plots <- lapply(genes_lineage, function(feature) {
  FeaturePlot(sobj_spleen5, features = feature)
})

# Combine plots into a grid
CombinePlots(plots = feature_plots, ncol = 3)

FeaturePlot(sobj_spleen5, c("Foxp3", "Il2ra"))
FeaturePlot(sobj_spleen5, c("Cd4", "Cd8a", "Foxp3"))
FeaturePlot(sobj_spleen5, c("Itgax"))
FeaturePlot(sobj_spleen5, c("Itgam")) #, "C1qb", "Itgam"))

FeaturePlot(sobj_spleen5, c("Cd14", "Ccr2", "Sell")) # Monocytes
FeaturePlot(sobj_spleen5, c("Cd163", "Cd68", "Mrc1"))
```
...
exp_data <- as.matrix(GetAssayData(sobj_spleen5, assay="RNA", layer="counts"))
norm_exp_data <- as.matrix(GetAssayData(sobj_spleen5, assay="RNA", layer="data"))
class(exp_data)
...
DefaultBoundary(sobj_spleen5[["fov31"]]) <- "segmentation" # "segmentation" or "centroids" # sets cell segmentation outline
DefaultBoundary(sobj_spleen5[["fov32"]]) <- "segmentation" # sets cell segmentation outline
DefaultBoundary(sobj_spleen5[["fov34"]]) <- "segmentation" # sets cell segmentation outline
DefaultBoundary(sobj_spleen5[["fov35"]]) <- "segmentation"
...
sobj_spleen5[[fov_name]] <- cropped.coords
DefaultBoundary(sobj_spleen5[[fov_name]]) <- "segmentation" 

ImageDimPlot(sobj_spleen5, fov="fov285", axes=TRUE, molecules= "Mki67", flip_xy = FALSE) #dark.background = FALSE)
...
sobj_spleen5[[zoom_name]] <- cropped.coords
DefaultBoundary(sobj_spleen5[[zoom_name]]) <- "segmentation" 

cols <- c("red", "blue", "pink", "green", "purple")
ImageDimPlot(sobj_spleen5, fov="zoom285", axes=TRUE, 
             molecules= c("Cdkna1","Mki67","Lmna", "Ccl5","Cxcl13"), 
             mols.size = 2.0,
